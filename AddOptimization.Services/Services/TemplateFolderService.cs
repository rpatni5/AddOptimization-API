    ...
    public class TemplateFolderService : ITemplateFolderService
    {
        ...
        private static TemplateEntryDto SelectEntityTemplate(TemplateEntries x, List<SharedEntry> sharedEntries, List<SharedFolder> sharedFolders , int currentUserId)
        {
            ...
            return new TemplateEntryDto
            {
                ...
                Permission = x.UserId == currentUserId? PermissionLevel.FullAccess.ToString() : entry != null ? entry.PermissionLevel: sharedFolder != null ? sharedFolder.PermissionLevel : PermissionLevel.FullAccess.ToString()
            };
        }
        ...
        public async Task<ApiResult<List<TemplateEntryDto>>> GetTemplates(Guid folderId)
        {
            try
            {
                ...
                var sharedEntries = (await _sharedEntryRepository.QueryAsync(x => !x.IsDeleted && (groupIds.Contains(x.SharedWithId)), include: entities => entities.Include(e => e.CreatedByUser).Include(e => e.UpdatedByUser))).ToList();
